...
function twemojify(text, createImgElement, opts) {
    ...
    var buildUrl = opts && opts.buildUrl;
    var emojis = (0, twemoji_parser_1.parse)(text, { assetType: assetType, buildUrl: buildUrl });
    ...
    emojis.forEach(function (emoji, idx) {
        ...
        position = end;
        var ampImgTag = createImgElement(__assign({ url: emoji.url, size: size, className: className, alt: alt, key: idx, style: style }, opts));
        if (str) {
            result.push(str);
        }
